...
async def _send_file_to_agent(from_agent_id: uuid.UUID, ws: Path, args: dict) -> str:
    ...
    try:
        ...
        async with async_session() as db:
            ...
            target_agent = None
            exact_result = await db.execute(select(Agent).where(Agent.name == agent_name, *base_filter))
            target_agent = exact_result.scalars().first()
            if not target_agent:
                # Sanitize SQL wildcards in user input
                safe_name = agent_name.replace("%", "").replace("_", r"\_")
                fuzzy_result = await db.execute(select(Agent).where(Agent.name.ilike(f"%{safe_name}%"), *base_filter))
...
async def _publish_page(agent_id: uuid.UUID, user_id: uuid.UUID, ws: Path, arguments: dict) -> str:
    ...
    try:
        ...
        async with async_session() as db2:
            r = await db2.execute(select(SystemSetting).where(SystemSetting.key == "platform"))
